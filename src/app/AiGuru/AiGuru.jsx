  ...
  const fetchAIQuestion = useCallback(async () => {
    ...
    try {
      const pastQuestions = chatHistory
        .filter((msg) => msg.role === "assistant" && !msg.text.includes("It is correct.") && !msg.text.includes("It is not correct"))
        .map((msg) => msg.text)
        .join("\n");
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.NEXT_PUBLIC_GEMINI_API_KEY}`,
      ...
      // Remove the previous user answer and assistant response, keeping only questions
      setChatHistory((prev) => {
        const filteredHistory = prev.filter(
          (msg) => msg.role === "assistant" && !msg.text.includes("It is correct.") && !msg.text.includes("It is not correct")
        );
        return [...filteredHistory, { role: "assistant", text: aiQuestion }];
      });
  ...
  const renderChatBubble = (msg, index) => (
    <motion.div
      ...
      className={`p-3 rounded-lg text-white ${
        msg.role === "user" ? "text-left w-full p-3" : "bg-gray-600 m-4"
      }`}
        ...
        >
          ...
          {answerHistory.length > 0 && (
            <div className="p-6 rounded-2xl mt-4 shadow-lg">
              <h3 className="text-xl font-semibold mb-4 text-gray-200">Previous Answers</h3>
                ...
                {answerHistory.map((item, index) => (
                  <div key={index} className="border-b border-gray-700 pb-4 last:border-0">
                    <p className="text-sm text-gray-300">
                      ...
                      <p dangerouslySetInnerHTML={{ __html: formatText(item.question) }}></p>
                    </p>
                    <p className="text-sm text-gray-300">
        ...
        <div className="mt-6 bg-[#36383A] border border-gray-500 rounded-3xl p-2 shadow-xl">
          ...
          <div className="flex items-center flex-col justify-between gap-4 max-md:flex-col">
            <input
              ...
              placeholder={inputMode === "topic" ? "Enter a Topic or Chapter Name..." : "Your answer..."}
              className="w-full p-3 rounded-xl text-white placeholder-gray-300 border-none outline-none transition-all duration-200"
              onKeyDown={(e) => inputMode === "answer" && e.key === "Enter" && checkAnswer()}
            />
            ...
            <div className="w-full">
              <div className="flex w-full gap-3 items-center justify-between">
                ...
                >
                  {isLoading ? <IoCreateOutline className="" size={20} /> : <FaArrowUp size={18} />}
                </motion.button>
              </div>
            </div>
          </div>
          ...
          <p className="text-xs text-center text-gray-400 mt-4 hidden max-lg:block">
            Explore AI in education with the best free AI tools for students. Get AI for research
            papers, plagiarism checking, and smart study solutions for better learning!
          </p>
        </div>
      </div>
    </div>
