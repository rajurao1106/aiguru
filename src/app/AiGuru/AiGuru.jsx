  ...
  const QuestionAnyTopic = () => {
    ...
    const fetchDefinition = useCallback(async () => {
      ...
      setError(null);
      setIsLoading(true);
      setShowButtons(false); // Hide buttons when fetching definition
    ...
    return (
      <>
        <div className="min-h-screen flex flex-col items-center justify-center p-8 text-white bg-blue-950 relative">
          ...
          {/* Main Card */}
          <div className="p-4 w-full max-w-lg bg-gray-800/80 rounded-2xl shadow-lg z-10 backdrop-blur-md">
            {/* Definition Display */}
            {definition && (
              <motion.p
                className="mt-4 p-4 overflow-y-auto h-[40vh] text-white bg-gray-800 
                ...
                dangerouslySetInnerHTML={{
                  __html: definition
                    ...
                    .replace(
                      /`([^`]+)`/g,
                      "<code class='bg-gray-700 p-1 rounded'>$1</code>"
                    ) // Inline Code
                    .replace(
                      /### (.*?)\n/g,
                    ...
                    .replace(
                      /# (.*?)\n/g,
                      "<h1 class='text-3xl font-extrabold'>$1</h1>"
                    ) // H1
                    .replace(
                      /\n- (.*?)\n/g,
                      "<ul class='list-disc ml-5'><li>$1</li></ul>"
                    ) // Unordered List
                    .replace(
                      /\n\d+\. (.*?)\n/g,
            ...
            >
              {isLoading ? "Wait For Answer..." : "Ask Question To Guru"}
            </motion.button>
            ...
            {/* Check Answer Button */}
            <motion.button
              className="mt-4 mb-4 w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 rounded-lg shadow-md transition-all"
              onClick={checkAnswer}
              whileHover={{ scale: 1.05 }}
            >
              Check Answer
            ...
            <div className="flex gap-2 ">
              ...
              <motion.button
                ...
                disabled={isLoading}
                whileHover={{ scale: 1.05 }}
                className=" bg-blue-600 hover:bg-blue-700 disabled:bg-gray-500 p-4 rounded-lg font-semibold transition-all"
              >
